...
import { formRegistroHorasSchema, FormRegistroHorasSchema } from '@components/FormRegistroHoras/schemas/formRegistroHorasSchema';
import { useState } from 'react';

// Props que o hook pode precisar (ex: se a submissão depender de algo externo)
interface UseFormRegistroHorasProps {
    // Se precisar, adicione props aqui, por exemplo, uma função de callback pós-submissão.
}
...
export function useFormRegistroHoras(/* props: UseFormRegistroHorasProps */) {
    ...
    const submitForm: SubmitHandler<FormRegistroHorasSchema> = async (data) => {
        ...
        setIsUploading(false);
        toast.success(`Horas de ${tipoRegistro === 'horas-extensao' ? 'Extensão' : 'Complementares'} registradas com sucesso!`);
