  ...
  'inline-flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-full text-base font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        outline: 'border border-[#1351B4] bg-white text-[#1351B4] hover:bg-[#D9E3F3]',
        ghost: 'bg-transparent text-[#1351B4] hover:bg-[#D9E3F3]',
        destructive: 'bg-transparent border border-red-600 text-red-600 hover:bg-red-100 focus-visible:ring-red-500',
  ...
  ({ className, variant, size, icon, asChild = false, children, ...props }, ref) => {
    ...
    return (
      <Comp className={cn(buttonVariants({ variant, size, className }))} ref={ref} {...props}>
        {icon && <FontAwesomeIcon icon={icon} className={children ? 'mr-1' : undefined} />}
        {children}
      </Comp>
    );
  }
